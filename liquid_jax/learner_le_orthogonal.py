...
class LeMnistLearner(Learner[LEInfo]):
    ...
    def boot_from_trial(
        ...
        def suggest_block(name: str, mindepth: int, maxdepth: int, maxwidth: int):
            ...
            block = [trial.suggest_int(f"{name}_layer_{i}", 1, maxwidth, log=True) for i in range(depth)]
            block = tuple(block)
            return block
        
        n_models = trial.suggest_int("n_models", 2, 128, log=True)
        body = suggest_block("body", mindepth=0, maxdepth=5, maxwidth=128)
        out = suggest_block("out", mindepth=0, maxdepth=4, maxwidth=128)
        body = suggest_block("delegation", mindepth=0, maxdepth=4, maxwidth=128)
        ...
        return Le(
            ...
            solver=LEsolver(
                load_distribution_lambda=trial.suggest_float("load_distribution_lambda", 0.0, 5.0, log=True),
                load_distribution_temperature=trial.suggest_float("load_distribution_temperature", 1e-8, 2.0),
                specialization_lambda=trial.suggest_float("specialization_lambda", 0.0, 5.0, log=True),
